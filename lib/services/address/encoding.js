...
/**
 * Attempts to retrieve address info from an input script.
 * @param {Script} - An instance of a Bitcore Script
 */
exports.extractAddressInfoFromInputScript = function(script) {
  var hashBuffer;
  var addressType;
  var hashTypeBuffer;
  if (script.isPublicKeyHashIn()) {
    hashBuffer = Hash.sha256ripemd160(script.chunks[1].buf);
    hashTypeBuffer = constants.HASH_TYPES.PUBKEY;
    addressType = Address.PayToPublicKeyHash;
  } else if (script.isScriptHashIn()) {
    hashBuffer = Hash.sha256ripemd160(input.script.chunks[input.script.chunks.length - 1].buf);
    hashTypeBuffer = constants.HASH_TYPES.REDEEMSCRIPT;
    addressType = Address.PayToScriptHash;
  } else {
    return false;
