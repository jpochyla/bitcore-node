...
/**
 * Will process each input of a transaction from the daemon "tx" event, and construct
 * an object with the data for the message to be relayed to any subscribers for an address.
 *
 * @param {Object} messages - An object to collect messages
 * @param {Transaction} tx - Instance of the transaction
 ...
 * @param {Boolean} rejected - If the transaction was rejected by the mempool
 */
AddressService.prototype.transactionInputHandler = function(messages, tx, inputIndex, rejected) {
  var script = tx.inputs[inputIndex].script;
  ...
  var addressInfo = encoding.extractAddressInfoFromInputScript(script);
  if (!addressInfo) {
    return;
  }
  ...
  // Collect data to publish to address subscribers
  if (messages[addressInfo.hashHex]) {
    messages[addressInfo.hashHex].inputIndexes.push(inputIndex);
  } else {
    messages[addressInfo.hashHex] = {
      tx: tx,
      outputIndexes: [],
...
AddressService.prototype.transactionHandler = function(txInfo, callback) {
  ...
  for (var i = 0; i < tx.inputs.length; i++) {
    this.transactionInputHandler(messages, tx, i, !txInfo.mempool);
  }
...
AddressService.prototype.blockHandler = function(block, addOutput, callback) {
  ...
  for (var i = 0; i < transactionLength; i++) {
    ...
    for(var inputIndex = 0; inputIndex < inputs.length; inputIndex++) {
      ...
      var inputKey = encoding.encodeInputKey(addressInfo.hashBuffer, addressInfo.hashTypeBuffer,
                                             height, prevTxIdBuffer, input.outputIndex);
      ...
      // Collect data for subscribers
      if (txmessages[addressInfo.hashHex]) {
        txmessages[addressInfo.hashHex].inputIndexes.push(inputIndex);
      } else {
        txmessages[addressInfo.hashHex] = {
          tx: tx,
          height: height,
